...
HPA / CPTAC / single-cell / spatial / TMA retrieval adapter, a runner, or an
LLM / embedding model. It never constructs a canonical ``CandidateGateAssessment``
or a ``Decision``; its output is a set of Gate-neutral ``EvidencePackage``
objects, one typed ``CrcCohortCoverageCompletion``, a machine-local
``fatal_review`` review TRIGGER, and one non-canonical
...
def run(
    ...
    *,
    provider: Tgt02CoverageProviderPort,
    ...
    (``module_input.candidate_id``, TGT-02). Nothing is persisted."""

    if not isinstance(module_input, Tgt02ModuleInput):
        raise TypeError("module_input must be a Tgt02ModuleInput")
    ...
    completion = provider.coverage_completion(
        candidate_id=module_input.candidate_id,
        target_identity=target_identity,
        run_id=run_id,
    )
    if not isinstance(completion, CrcCohortCoverageCompletion):
        raise TypeError(
            "provider.coverage_completion must return a CrcCohortCoverageCompletion"
        )
    ...
    # --- E8-5 invariant 3: qualifying cohort-set parity ------------------
    # "qualifying" is defined only over a COMPLETED audited landscape (E7 item 06
    # qualifying_is_rung_specific). On an incomplete landscape nothing is
    # qualifying yet, so there is nothing to reconcile.
    if completion.landscape_complete:
        direct_cohort_ids: set[str] = set()
        indirect_cohort_ids: set[str] = set()
        for e in emitted:
            if not e.classified.admissible:
                continue
            if e.classified.qualifying_for_direct:
                direct_cohort_ids.update(e.observation.cohort_identities)
            elif e.classified.qualifying_for_indirect:
                indirect_cohort_ids.update(e.observation.cohort_identities)
        why = qualifying_set_mismatch(
            completion,
            direct_cohort_ids=direct_cohort_ids,
            indirect_cohort_ids=indirect_cohort_ids,
        )
        if why:
            hard_integrity_failures.append(("coverage_completion", why))
            rejected_records.append(("coverage_completion", why))
    ...
    if accepted:
        try:
            proposal_envelope = AssessmentProposalEnvelope(
                ...
                evidence_ceiling=TGT02_EVIDENCE_CEILING,
            )
    ...
    return Tgt02ModuleRunResult(
        ...
        coverage_completion=completion,
