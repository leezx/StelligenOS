...
审核豁免的单位是**整个 PR**，不是单个文件。豁免 PR 的改动必须完全落在以下三个路径之内，一个都不能多：

| 路径 | 数量与写法 | 角色 |
|---|---|---|
| `prompts/GPT-Feedback.md` | 恰好 1 | **反馈正文**，本豁免的对象 |
| `logs/worklog.md` | 恰好 1，只允许在文件末尾追加一条带时间戳的条目 | 必需的配套审计文件 |
| `docs/handoff/<日期>-<任务名>.zh-CN.md` | 恰好 1，新增文件 | 必需的配套审计文件 |

**worklog 与 handoff 是豁免 PR 必需的配套审计文件。它们的出现不触发正常审核门禁**——三者同时出现的
PR 整体仍然默认通过。但它们只能承载上表规定的内容：worklog 只能追加一条记录本次反馈更新的条目，
handoff 只能描述本次反馈更新本身。**在这两个文件里写入与本次反馈更新无关的内容，即视为超出允许集合，
整个 PR 回落到正常门禁。**

因此本节没有修改任何留痕要求。第 26 条、
`docs/protocols/CHATGPT_CODEX_PHASE_GATE_PROTOCOL.zh-CN.md` 与 `ChatGPT-Codex-talk.md` 中「每个 PR
都必须更新 `docs/handoff/`」一律照常适用；被豁免的只是审核，配套留痕仍是强制的。

出现上表三者之外的任何改动——包括 `prompts/` 下其他文件、任何 `.py`、任何契约或 manifest——整个 PR
...
2. **留痕照常。** 仍须走 PR、仍须按上表写 worklog 与 handoff。被豁免的是审核，不是可追溯性。
3. **实施不在豁免范围内。** 把该文件里的反馈变成架构、内核、Gate、扩展或代码改动，一律走完整门禁。
   记录反馈 ≠ 获得实施授权，这一点与 `extensions/README.md`「反馈本身不是架构变更授权」一致。
4. **不能自我扩展。** 本豁免不适用于修改 `AGENTS.md` 本身，也不适用于扩大本豁免的范围。用「默认通过」
