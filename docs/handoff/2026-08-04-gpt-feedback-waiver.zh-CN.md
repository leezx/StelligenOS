...
审核豁免的单位是**整个 PR**，不是单个文件。豁免 PR 的改动必须完全落在三个路径之内：

| 路径 | 数量与写法 | 角色 |
|---|---|---|
| `prompts/GPT-Feedback.md` | 恰好 1 | **反馈正文**，本豁免的对象 |
| `logs/worklog.md` | 恰好 1，只允许末尾追加一条带时间戳条目 | 必需的配套审计文件 |
| `docs/handoff/<日期>-<任务名>.zh-CN.md` | 恰好 1，新增 | 必需的配套审计文件 |

worklog 与 handoff 是豁免 PR 的**必需配套审计文件**，它们的出现**不触发正常审核门禁**；三者同时出现的
PR 整体仍默认通过。但它们只能承载规定内容——worklog 只能追加一条记录本次反馈更新的条目，handoff 只能
描述本次反馈更新本身；写入无关内容即视为超出允许集合，整个 PR 回落正常门禁。

### 其余四条边界

1. **只限这一个反馈文件**，不含 `prompts/` 下任何其他文件。
2. **留痕照常**，仍须走 PR、仍须按上表写 worklog 与 handoff。
3. **实施不在豁免范围内**。把该文件的反馈变成架构、内核、Gate、扩展或代码改动，一律走完整门禁。记录反馈
   ≠ 获得实施授权，与 `extensions/README.md`「反馈本身不是架构变更授权」一致。
4. **不能自我扩展**，不适用于修改 `AGENTS.md` 本身或扩大本豁免。

文件集合表与第 3、4 条各堵一个实际会出现的口子：
...
ChatGPT 返回 `REQUEST_CHANGES`，一条治理语义冲突，经核实成立。

### 阻断：把「是否豁免」按文件逐行标注，使豁免自我作废

成立。Round 1 修订虽然封闭了文件集合，但表格设了一列「是否豁免」，把 `logs/worklog.md` 与 handoff 标为
**不豁免**，并写「handoff 不在豁免之列」。

**PR 审核以整个 PR 为单位。** 若配套文件「不豁免」，则含这两个文件的 PR 仍需审核，而按上表这两个文件是
必需的——于是每一个合规的豁免 PR 都需要审核，豁免恒不生效。这不是措辞瑕疵，是逻辑上把自己否掉了。
...
「handoff 必须写」是**留痕**轴上的真命题，我却把它写成了**审核**轴上的「handoff 不豁免」，两者含义完全
不同。

修订按审核意见执行：

- 表格「是否豁免」一列改为「**角色**」，取值为「反馈正文」与「必需的配套审计文件」，不再出现按文件标注
  的豁免与否。
- 明确写出「worklog 与 handoff 是豁免 PR 必需的配套审计文件，**它们的出现不触发正常审核门禁**」，并限定
  只能承载规定内容，写入无关内容即超出允许集合。
- 四条边界中含「不豁免」字样的表述改写：「不豁免留痕」→「留痕照常」，「不豁免实施」→「实施不在豁免范围
  内」，使「豁免」一词只用于 PR 层面。
- `ChatGPT-Codex-talk.md` 与 Phase Gate 协议中的指针原写「不豁免 PR、worklog 与 handoff」，同属混轴表述，
  一并改为「只豁免审核；PR、worklog 与 handoff 仍须照常提交，且这两个配套文件的出现不会使该 PR 重新落入
  审核门禁」。三处表述至此一致。

### 审核确认已成立的部分

三文件集合已封闭；`ChatGPT-Codex-talk.md` 与 Phase Gate 协议已同步例外。
...
结果：Ran 207 tests —— OK
      （Round 1 时为 192；PR #50 已合并进 `main`，本分支已并入其 15 项边界测试）
...
- PR #50（CI 与依赖声明）已获 `APPROVE` 并合并为 `927aebf`。本分支已并入合并后的 `main`，唯一冲突为
  `logs/worklog.md` 的追加式冲突，已按时间戳顺序保留两侧。本 PR 现在也受 CI 覆盖。
