...
- 当前架构说明审核基线：`STELLIGENOS-ARCH-2026.08.27-v5`（PR #94 `APPROVE`；依据 Blueprint v1.3 深度对齐 Candidate × Gate × Evidence 六对象决策模型；runtime conformance = **COMPLETE**，Blueprint-v1.3 Candidate × Gate × Evidence runtime migration（PR A–E16）已收口，8 个 primary Evidence Production Module 全部建成；其余 deferred work（quantitative ladder calibration、epitope-layer 分析、external evaluators、下游 Candidate level、FTO 任务）仍在后续）
